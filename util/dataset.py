...
class SemData_SP(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # For every support label, making the corresponding initial sp seed: b x shot x num_sp x 2
        fg_list, bg_list = [], []
        init_fg_seed_list, init_bg_seed_list = [], []
        for i in range(0, self.shot):
            fg_mask = (s_y[i, :, :] == 1).float()   # H x W
            bg_mask = (s_y[i, :, :] == 255).float() # H x W
            fg_seed = place_seed_points(fg_mask, down_stride=8, max_num_sp=self.max_sp, avg_sp_area=100)
            bg_seed = place_seed_points(bg_mask, down_stride=8, max_num_sp=self.max_sp, avg_sp_area=100)
            init_fg_seed_list.append(fg_seed.unsqueeze(0))
            init_bg_seed_list.append(bg_seed.unsqueeze(0))
        s_fg_seed = torch.cat(init_fg_seed_list, 0)  # (shot, max_num_sp, 2)
        s_bg_seed = torch.cat(init_bg_seed_list, 0)  # (shot, max_num_sp, 2)
        
        return image, label, s_x, s_y, s_fg_seed, s_bg_seed, subcls_list

        # if self.mode == 'train':
        #     return image, label, s_x, s_y, s_fg_seed, s_bg_seed, subcls_list
        # else:
        #     return image, label, s_x, s_y, s_fg_seed, s_bg_seed, subcls_list, raw_label
